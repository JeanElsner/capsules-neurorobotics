...
for i in range(n_train):
    ...
    choice = torch.bernoulli(0.01 * torch.rand(1, 25, 4, 4))
    clamp = torch.zeros(time, 25, 4, 4, dtype=torch.uint8)
    clamp[:] = choice.byte()
    clamp = {'Y' : clamp}
    clamp_v = torch.zeros(time, 25, 4, 4, dtype=torch.float)
    clamp_v[:, choice.byte().squeeze()] = network.layers['Y'].thresh + 10#network.layers['Y'].theta[choice.byte()] + 10
    network.run(inpts=inpts, time=time, clamp=clamp, clamp_v={'Y': clamp_v})

    # Optionally plot various simulation information.
    if plot:
        inpt = inpts['X'].view(time, 784).sum(0).view(28, 28)
        #weights1 = conv_conn.w
        weights1 = network.connections[('X', 'Y')].w
        print(weights1)
