...
def main(seed=0, n_neurons=100, n_train=60000, n_test=10000, inhib=100, lr_decay=1, time=350, dt=1, theta_plus=0.05,
    ...
    if train:
        network = DiehlAndCook2015v2(
            n_inpt=32**2, n_neurons=n_neurons, inh=inhib, dt=dt, norm=norm, theta_plus=theta_plus,
            theta_decay=theta_decay, nu_pre=0, nu_post=np.sqrt(350 / float(time)) * lr
        )
    ...
    dataset = VPR('./data/Dataset_lighting4/left')
    ...
    if train:
        ...
        proportions = torch.zeros_like(torch.Tensor(n_neurons, 5))
        rates = torch.zeros_like(torch.Tensor(n_neurons, 5))
        ngram_scores = {}
    else:
    ...
    for i in range(n_examples):
        ...
        if i % update_interval == 0 and i > 0:
            ...
            if train:
                ...
                assignments, proportions, rates = assign_labels(spike_record, current_labels, 5, rates)

                # Compute ngram scores.
                ngram_scores = update_ngram_scores(spike_record, current_labels, 5, 2, ngram_scores)
        ...
        if plot:
            ...
            square_weights = get_square_weights(input_exc_weights.view(32**2, n_neurons), n_sqrt, 32)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_neurons', type=int, default=500, help='no. of output layer neurons')
    parser.add_argument('--n_train', type=int, default=4010, help='no. of training samples')
    parser.add_argument('--n_test', type=int, default=2412, help='no. of test samples')
    parser.add_argument('--inhib', type=float, default=500.0, help='inhibition connection strength')
    parser.add_argument('--lr', type=float, default=1e-2)
    parser.add_argument('--norm', type=float, default=32**2/5)
    parser.add_argument('--lr_decay', type=float, default=.9, help='rate at which to decay learning rate')
    parser.add_argument('--time', default=200, type=int, help='simulation time')
    parser.add_argument('--dt', type=float, default=1, help='simulation integreation timestep')
    parser.add_argument('--theta_plus', type=float, default=0.5, help='adaptive threshold increase post-spike')
