...
for i, (datum, label) in enumerate(loader):
    ...
    #weights_im = plot_weights(get_square_weights(C1.w, 23, 32), im=weights_im, wmin=-2, wmax=2)
...
for epoch in range(10):
    for i, (s, label) in enumerate(training_pairs):
        ...
        if (i + 1) % 100 == 0:
            print(
                'Epoch: [%d/%d], Step: [%d/%d], Loss: %.4f' % (epoch + 1, 10, i + 1, len(training_pairs), loss.data[0]))
