...
parser.add_argument('--data-dir', type=str, default=r'C:\Users\jeane\Desktop\Experimentation\examples\left', metavar='DD',
                    help='where to store the datasets')
parser.add_argument('--dataset', type=str, default='VPR', metavar='D',
                    help='dataset for training(mnist, VPR)')
...
def get_setting(args):
    ...
    elif args.dataset == 'VPR':
        num_class = 5
        train_loader = get_VPR_train_data(path, args.batch_size, cuda=args.cuda, shuffle=True)
        test_loader = get_VPR_test_data(path, args.test_batch_size, cuda=args.cuda, shuffle=True)
    else:
        ...
        for epoch in range(args.start_epoch, args.epochs + 1):
            ...
            if epoch % args.test_intvl == 0:
                ...
                if test_acc > best_acc:
                    best_acc = test_acc
                    snapshot(model, args.snapshot_dir, epoch)
